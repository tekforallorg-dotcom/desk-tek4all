...
export default function MessagingPage() {
  ...
  const getConversationTitle = (conv: Conversation) => {
    ...
    if (conv.participants && conv.participants.length > 0) {
      ...
      const names = conv.participants
        .slice(0, 2)
        .map((p) => p.user.full_name?.split(" ")[0] || p.user.username);
  ...
  const getConversationIcon = (conv: Conversation) => {
    if (
      conv.type === "group" ||
      (conv.participant_count && conv.participant_count > 2)
    ) {
      return Users;
    }
    switch (conv.type) {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-5xl flex-col">
      ...
      <div className="space-y-3 pb-4 sm:flex sm:items-center sm:justify-between sm:space-y-0 sm:pb-6">
        <div>
      ...
      {/* Conversation List */}
      <div className="flex-1 overflow-hidden rounded-2xl border-2 border-border bg-card shadow-retro">
        <div className="h-full overflow-y-auto">
          {isLoading ? (
            <div className="space-y-0 divide-y-2 divide-border">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-3 p-4 sm:gap-4">
                  <div className="h-10 w-10 animate-pulse rounded-xl bg-muted sm:h-12 sm:w-12" />
              ...
              {filteredConversations.map((conv) => {
                ...
                return (
                  <Link key={conv.id} href={`/messaging/${conv.id}`}>
                    <div className="flex items-center gap-3 p-3 transition-colors hover:bg-muted/50 sm:gap-4 sm:p-4">
                      {/* Avatar */}
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border-2 border-border bg-background sm:h-12 sm:w-12">
                        <Icon
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex min-w-0 items-center gap-2">
                            ...
                            {subtitle && (
                              <span className="hidden shrink-0 rounded-full border border-border bg-background px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground sm:inline">
                                {subtitle}
                              </span>
                            )}
                          </div>
                        ...
                        {conv.last_message && (
                          <p className="mt-0.5 truncate text-sm text-muted-foreground">
                            <span className="font-medium">
                              {conv.last_message.sender?.full_name?.split(
                                " "
                              )[0] || conv.last_message.sender?.username}
                              :
                            </span>{" "}
...
function NewMessageModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-t-2xl border-2 border-border bg-card p-4 shadow-retro-lg sm:rounded-2xl sm:p-6">
            ...
            <div className="max-h-36 overflow-y-auto rounded-xl border-2 border-border">
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.slice(0, 10).map((u) => (
                  ...
                  >
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border-2 border-border bg-background font-mono text-xs font-bold">
